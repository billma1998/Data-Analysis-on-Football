...
path = "F:/FootballAnalysis/football data/"
database = path + 'database.sqlite'
data = sqlite3.connect(database)

tables = pd.read_sql("""SELECT * FROM sqlite_master WHERE type='table';""", data)

teams = pd.read_sql("""SELECT * FROM Team ORDER BY team_long_name LIMIT 10;""", data)

leagues = pd.read_sql("""SELECT *FROM League JOIN Country ON Country.id = League.country_id;""", data)
countries = pd.read_sql("""SELECT * FROM Country;""", data)
